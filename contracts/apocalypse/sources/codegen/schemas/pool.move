module apocalypse::pool_schema {
	...
	public fun new(balance: u64, staker_balance: u64, staker_balance_plus: u64, player_balance: u64, player_balance_plus: u64, founder_balance: u64, prop_mint_fee: u64, game_fee: u64, min_prop_fee: u64, prop_burn_fee: u64, to_staker_fee: u64, to_player_fee: u64, scissors_count: u64, rock_count: u64, paper_count: u64, prop_count: u64): PoolData {
		PoolData {
			balance, 
			staker_balance, 
			staker_balance_plus, 
	...
	public(friend) fun set_game_fee(_obelisk_world: &mut World, game_fee: u64) {
		let _obelisk_schema = world::get_mut_schema<PoolData>(_obelisk_world, SCHEMA_ID);
		_obelisk_schema.game_fee = game_fee;
		events::emit_set(SCHEMA_ID, SCHEMA_TYPE, none(), *_obelisk_schema)
	}

	public(friend) fun set_min_prop_fee(_obelisk_world: &mut World, min_prop_fee: u64) {
		let _obelisk_schema = world::get_mut_schema<PoolData>(_obelisk_world, SCHEMA_ID);
		_obelisk_schema.min_prop_fee = min_prop_fee;
		events::emit_set(SCHEMA_ID, SCHEMA_TYPE, none(), *_obelisk_schema)
	}

	public(friend) fun set_prop_burn_fee(_obelisk_world: &mut World, prop_burn_fee: u64) {
		let _obelisk_schema = world::get_mut_schema<PoolData>(_obelisk_world, SCHEMA_ID);
		_obelisk_schema.prop_burn_fee = prop_burn_fee;
		events::emit_set(SCHEMA_ID, SCHEMA_TYPE, none(), *_obelisk_schema)
	}

	public(friend) fun set_to_staker_fee(_obelisk_world: &mut World, to_staker_fee: u64) {
		let _obelisk_schema = world::get_mut_schema<PoolData>(_obelisk_world, SCHEMA_ID);
		_obelisk_schema.to_staker_fee = to_staker_fee;
		events::emit_set(SCHEMA_ID, SCHEMA_TYPE, none(), *_obelisk_schema)
	}

	public(friend) fun set_to_player_fee(_obelisk_world: &mut World, to_player_fee: u64) {
		let _obelisk_schema = world::get_mut_schema<PoolData>(_obelisk_world, SCHEMA_ID);
		_obelisk_schema.to_player_fee = to_player_fee;
		events::emit_set(SCHEMA_ID, SCHEMA_TYPE, none(), *_obelisk_schema)
	}
	...
	public fun get(_obelisk_world: &World): (u64,u64,u64,u64,u64,u64,u64,u64,u64,u64,u64,u64,u64,u64,u64,u64) {
	...
	public fun get_game_fee(_obelisk_world: &World): u64 {
		let _obelisk_schema = world::get_schema<PoolData>(_obelisk_world, SCHEMA_ID);
		_obelisk_schema.game_fee
	}

	public fun get_min_prop_fee(_obelisk_world: &World): u64 {
		let _obelisk_schema = world::get_schema<PoolData>(_obelisk_world, SCHEMA_ID);
		_obelisk_schema.min_prop_fee
	}

	public fun get_prop_burn_fee(_obelisk_world: &World): u64 {
		let _obelisk_schema = world::get_schema<PoolData>(_obelisk_world, SCHEMA_ID);
		_obelisk_schema.prop_burn_fee
	}

	public fun get_to_staker_fee(_obelisk_world: &World): u64 {
		let _obelisk_schema = world::get_schema<PoolData>(_obelisk_world, SCHEMA_ID);
		_obelisk_schema.to_staker_fee
	}

	public fun get_to_player_fee(_obelisk_world: &World): u64 {
		let _obelisk_schema = world::get_schema<PoolData>(_obelisk_world, SCHEMA_ID);
		_obelisk_schema.to_player_fee
	}
