module apocalypse::card_map_schema {
	use std::option::some;
	...
	const SCHEMA_ID: vector<u8> = b"card_map";
	const SCHEMA_TYPE: u8 = 0;

	// value
	struct CardMapData has copy, drop , store {
		value: vector<address>
	}

	public fun new(value: vector<address>): CardMapData {
		CardMapData {
			value
		}
	}
	...
	public fun register(_obelisk_world: &mut World, admin_cap: &AdminCap, ctx: &mut TxContext) {
		world::add_schema<Table<address,CardMapData>>(_obelisk_world, SCHEMA_ID, table::new<address, CardMapData>(ctx), admin_cap);
	}

	public(friend) fun set(_obelisk_world: &mut World, _obelisk_entity_key: address,  value: vector<address>) {
		let _obelisk_schema = world::get_mut_schema<Table<address,CardMapData>>(_obelisk_world, SCHEMA_ID);
		let _obelisk_data = new( value);
		if(table::contains<address, CardMapData>(_obelisk_schema, _obelisk_entity_key)) {
			*table::borrow_mut<address, CardMapData>(_obelisk_schema, _obelisk_entity_key) = _obelisk_data;
		} else {
	...
	public fun get(_obelisk_world: &World, _obelisk_entity_key: address): vector<address> {
		let _obelisk_schema = world::get_schema<Table<address,CardMapData>>(_obelisk_world, SCHEMA_ID);
		assert!(table::contains<address, CardMapData>(_obelisk_schema, _obelisk_entity_key), EEntityDoesNotExist);
		let _obelisk_data = table::borrow<address, CardMapData>(_obelisk_schema, _obelisk_entity_key);
		(
			_obelisk_data.value
		)
	}
	...
	public fun contains(_obelisk_world: &World, _obelisk_entity_key: address): bool {
		let _obelisk_schema = world::get_schema<Table<address,CardMapData>>(_obelisk_world, SCHEMA_ID);
		table::contains<address, CardMapData>(_obelisk_schema, _obelisk_entity_key)
	}
}
